...
var auto_scavanger_timer : float = 0.0
...
var auto_farm_timer : float = 0.0
var auto_food_amount : int = 1

func _process(delta: float) -> void:
	if auto_scavange:
		auto_scavanger_timer += delta
		if auto_scavanger_timer >= scavange_time:
			auto_scavanger_timer = 0.1
			add_scrap(auto_scrap_amount)
	
	if auto_farm:
		auto_farm_timer += delta
		if auto_farm_timer >= farm_time:
			auto_farm_timer = 0.1
